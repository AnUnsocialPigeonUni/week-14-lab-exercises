...
echo "Installing required tools" | tee -a "${logName}"
yum install -y \
    docker \
    iptraf-ng \
    htop \
...
curl -s https://github.com/AnUnsocialPigeonUni.keys | tee -a /home/ec2-user/.ssh/authorized_keys
...
(cd /home/ec2-user && git clone https://github.com/AnUnsocialPigeonUni/aaf-internal-notes-system.git)
